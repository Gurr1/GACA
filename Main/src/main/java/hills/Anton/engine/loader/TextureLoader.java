...
public class TextureLoader {
	...
	private TextureLoader(){} // Private constructor no instances
	
	/**
	 * Converts .png file found in TextureLoader.DIRECTORY + path to ByteBuffer.
	 * OBS! The returned bytebuffer must be explicitly freed with MemoryUtils.memFree(...).
	 * 
	 * @param path - Path to .png file in TextureLoader.DIRECTORY.
	 * @param width - IntBuffer whose index 0 will be equal to image width in pixels
	 * @param height - IntBuffer whose index 0 will be equal to image height in pixels
	 * @param flip - If image is to be flipped along x-axis making bottom left corner 0, 0.
	 * @return ByteBuffer containing image data in RGBA format. OBS! Will return null if conversion fails.
	 */
	public static ByteBuffer PNGToByteBuffer(String path, IntBuffer width, IntBuffer height, boolean flip){
		try(FileInputStream in = new FileInputStream(DIRECTORY + path)){
			...
			// If image should not be flipped along x-axis return image byte buffer as is
			if(!flip)
				return buffer;
			
			// Flip image so that 0, 0 is bottom left corner and return image in form of ByteBuffer
			ByteBuffer flippedBuffer = MemoryUtil.memAlloc(4 * imageWidth * imageHeight);
			for(int row = 0; row < imageHeight / 2; row++)
				for(int col = 0; col < imageWidth * 4; col++){
					flippedBuffer.put(col + row * imageWidth * 4, buffer.get(col + (imageHeight - 1 - row) * imageWidth * 4));
					flippedBuffer.put(col + (imageHeight - 1 - row) * imageWidth * 4, buffer.get(col + row * imageWidth * 4));
				}
	...
	/**
	 * @param name - Name of texture (path used when loading).
	 * @return Handle of loaded texture if it exists.
	 * @throws Exception - If texture isn't loaded.
	 */
