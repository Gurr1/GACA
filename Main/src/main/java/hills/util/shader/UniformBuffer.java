...
public class UniformBuffer {
	 ...
	 */
	private static final int MAX_BIND_POINTS = GL11
			.glGetInteger(GL31.GL_MAX_UNIFORM_BUFFER_BINDINGS);
	 ...
	 *            - Size of the buffer.
	 * @param varNames
	 *            - Names of the variables inside the buffer object.
	 * @param offsets
	 *            - Offsets of the variables inside the buffer object.
	 */
	protected UniformBuffer(int size, String[] varNames, int[] offsets) {
		...
		for (int variable = 0; variable < varNames.length; variable++) {
			...
			if ((nameEnd = varNames[variable].indexOf(']')) > 0) {
				String arrayVariableName = varNames[variable].substring(0,
						nameEnd + 1);
				Integer arrayVariableOffset = variableOffsets
						.get(arrayVariableName);
				if (arrayVariableOffset == null)
					arrayVariableOffset = offsets[variable] + 1; // Make sure
																	// offset
																	// gets
																	// selected
																	// from
				...
				variableOffsets
						.put(arrayVariableName,
								arrayVariableOffset < offsets[variable] ? arrayVariableOffset
										: offsets[variable]);
			}
		}
	 ...
	 * 
	 * @param values
	 *            - Values to map to buffer.
	 */
	public void map(byte[] values) {
		if (values.length != size) {
			System.err
					.println("Values sent to uniform buffer object must be of same size as the buffer! Size of buffer: "
							+ size
							+ " | Size of in values: "
							+ values.length
		...
		GL30.glMapBufferRange(GL31.GL_UNIFORM_BUFFER, 0, values.length,
				GL30.GL_MAP_WRITE_BIT | GL30.GL_MAP_INVALIDATE_BUFFER_BIT).put(
				values);
	...
	public void map(ByteBuffer values) {
		...
		GL30.glBindBufferBase(GL31.GL_UNIFORM_BUFFER, bindPoint, handle);
		GL30.glMapBufferRange(GL31.GL_UNIFORM_BUFFER, 0, values.remaining(),
				GL30.GL_MAP_WRITE_BIT | GL30.GL_MAP_INVALIDATE_BUFFER_BIT).put(
				values);
		GL15.glUnmapBuffer(GL31.GL_UNIFORM_BUFFER);
	}
	 ...
	 * 
	 * @param offset
	 *            - Start point of mapping in bytes.
	 * @param values
	 *            - Values to map from start point.
	 */
	public void map(int offset, byte[] values) {
		if (offset + values.length > size) {
			System.err
					.println("Values sent to uniform buffer object must be of same size as or smaller than the buffer!"
							+ bindPoint);
			return;
		}

		GL30.glBindBufferBase(GL31.GL_UNIFORM_BUFFER, bindPoint, handle);
		GL30.glMapBufferRange(GL31.GL_UNIFORM_BUFFER, offset, values.length,
				GL30.GL_MAP_WRITE_BIT | GL30.GL_MAP_INVALIDATE_RANGE_BIT).put(
				values);
		GL15.glUnmapBuffer(GL31.GL_UNIFORM_BUFFER);
	}
	...
	public void map(int offset, ByteBuffer values) {
		...
		GL30.glBindBufferBase(GL31.GL_UNIFORM_BUFFER, bindPoint, handle);
		GL30.glMapBufferRange(GL31.GL_UNIFORM_BUFFER, offset,
				values.remaining(),
				GL30.GL_MAP_WRITE_BIT | GL30.GL_MAP_INVALIDATE_RANGE_BIT).put(
				values);
		GL15.glUnmapBuffer(GL31.GL_UNIFORM_BUFFER);
	 ...
	 * 
	 * @param varName
	 *            - Name of variable to get offset of.
	 * @return Variable offset in buffer.
	 */
	public int getOffset(String varName) {
		Integer offset = variableOffsets.get(varName);
		if (offset == null)
			throw new IllegalArgumentException("Could not find variable "
					+ varName + " in uniform buffer!");
