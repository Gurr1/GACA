...
public final class Quaternion {
	 ...
	 * @param angle
	 *            - Degrees of rotation around rotation axis.
	 */
	 ...
	 * 
	 * @param vers
	 *            - the quaternion defined by a rotation axis (x, y, z) and an
	 *            angle in degrees (w).
	 */
	public Quaternion(Vec4 vers) {
	...
	public Quaternion(Vec3 axis, float halfAngleCos, float halfAngleSin) {
		float x = halfAngleSin * axis.getX();
		float y = halfAngleSin * axis.getY();
		float z = halfAngleSin * axis.getZ();
		float w = halfAngleCos;
	...
	public Quaternion inverse() {
		return new Quaternion(getVersor().mul(
				new Vec4(-1.0f, -1.0f, -1.0f, 1.0f)));
	}

	public Quaternion slerp(final Quaternion quat1, final Quaternion quat2,
			final float t) { // Spherical interpolation between 2 quaternions
		...
		if (dot > DOT_THRESHOLD) // If inputs very close, linearly interpolate +
									// normalize.
		...
		dot = Math.max(-1.0f, Math.min(1.0f, dot)); // Clamp dot product to stay
													// within domain of acos()
		float theta_0 = (float) Math.acos(dot); // theta_0 = angle between input
												// vectors
		float theta = theta_0 * t; // theta = angle between v0 and result

		Vec4 v3 = v2.sub(v1.mul(dot)).normalize(); // { v1, v3 } is now an
													// orthonormal basis
		return new Quaternion(v1.mul(((float) Math.cos(theta))).add(
				v3.mul((float) Math.sin(theta))));
	}
	...
	public Mat4 getMatrix() {
		Vec4 normVersor = versor.getLengthSqr() != 1.0f ? versor.normalize()
				: new Vec4(versor);
