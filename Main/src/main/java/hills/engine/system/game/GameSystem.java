...
public final class GameSystem extends EngineSystem {
	...
	Vertex[] v = {
			new Vertex(new Vec3(-.5f, -.5f, .5f), new Vec2(0.0f, 0.0f),
					new Vec3(0.0f, 0.0f, 1.0f)),
			new Vertex(new Vec3(-.5f, -.5f, .5f), new Vec2(0.0f, 1.0f),
					new Vec3(0.0f, -1.0f, 0.0f)),
			new Vertex(new Vec3(-.5f, -.5f, .5f), new Vec2(1.0f, 0.0f),
					new Vec3(-1.0f, 0.0f, 0.0f)),

			new Vertex(new Vec3(.5f, -.5f, .5f), new Vec2(1.0f, 0.0f),
					new Vec3(0.0f, 0.0f, 1.0f)),
			new Vertex(new Vec3(.5f, -.5f, .5f), new Vec2(1.0f, 1.0f),
					new Vec3(0.0f, -1.0f, 0.0f)),
			new Vertex(new Vec3(.5f, -.5f, .5f), new Vec2(0.0f, 0.0f),
					new Vec3(1.0f, 0.0f, 0.0f)),

			new Vertex(new Vec3(.5f, .5f, .5f), new Vec2(1.0f, 1.0f), new Vec3(
					0.0f, 0.0f, 1.0f)),
			new Vertex(new Vec3(.5f, .5f, .5f), new Vec2(1.0f, 0.0f), new Vec3(
					0.0f, 1.0f, 0.0f)),
			new Vertex(new Vec3(.5f, .5f, .5f), new Vec2(0.0f, 1.0f), new Vec3(
					1.0f, 0.0f, 0.0f)),
	...
	private GameSystem(float scale, boolean isPaused, float startTime) {
		...
		// WaterPlane waterPlane = new WaterPlane(new Vec3(1024.0f, 5.0f,
		// 1024.0f), new Vec2(2048.0f, 2048.0f), new Vec3(0.0f, 1.0f,
		// 0.0f).normalize());
		// WaterRenderer.INSTANCE.batch(waterPlane);
	}
	...
	@Override
	public void render() {
		// Renderer.batch(ShaderProgram.STATIC, cube,
		// Mat4.identity().scale(16.0f * 2, 16.0f * 2, 16.0f *
		// 2).translate(CameraSystem.getInstance().getPosition().mul(new
		// Vec3(1.0f, 0.0f, 1.0f))));
	}
	 ...
	 */
	public static boolean createInstance(float scale, boolean isPaused,
			float startTime) {
		if (instance != null)
			return false;
