...
public class CameraSystem extends EngineSystem {
	...
	private CameraSystem(float scale, boolean isPaused, float startTime) {
		...
		frustrum = new Frustrum(near, far, aspect, FOV, position, forward, up,
				right, false);

		try (MemoryStack stack = MemoryStack.stackPush()) {
			Mat4 identityMatrix = Mat4.identity();
			ByteBuffer dataBuffer = stack.calloc(identityMatrix
					.get140DataSize());
	 ...
	 *            - degrees to rotate.
	 * @param x
	 *            - X component of axis to rotate around.
	 * @param y
	 *            - Y component of axis to rotate around.
	 * @param z
	 *            - Z component of axis to rotate around.
	 */
	 ...
	 * 
	 * @param dir
	 *            - Direction to move in. If set to NONE camera will stop.
	 */
	...
	public void updatePerspective(float near, float far, float aspect, float FOV) {
		...
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			ByteBuffer dataBuffer = stack.calloc(perspectiveMatrix
					.get140DataSize());
