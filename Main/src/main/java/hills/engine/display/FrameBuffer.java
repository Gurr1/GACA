...
public class FrameBuffer {
	 ...
	 * @param internalFormat - The textures internal format.
	 ...
	 */
	public void attachTexture(int target, int internalFormat, int width, int height, int format, int attachment){
		bind();
		
		int handle = GL11.glGenTextures();
		GL11.glBindTexture(target,  handle);
		GL11.glTexImage2D(target, 0, internalFormat, width, height, 0, format, GL11.GL_UNSIGNED_BYTE, MemoryUtil.NULL);
		...
		if(target == GL11.GL_TEXTURE_1D)
			GL30.glFramebufferTexture1D(GL30.GL_FRAMEBUFFER, attachment, GL11.GL_TEXTURE_1D, handle, 0);
		else if(target == GL11.GL_TEXTURE_2D)
			GL30.glFramebufferTexture2D(GL30.GL_FRAMEBUFFER, attachment, GL11.GL_TEXTURE_2D, handle, 0);
		else if(target == GL12.GL_TEXTURE_3D)
			GL30.glFramebufferTexture3D(GL30.GL_FRAMEBUFFER, attachment, GL12.GL_TEXTURE_3D, handle, 0, 0);
	 ...
	 * @param internalFormat - The textures internal format.
	 * @param width - The render buffer width.
	 * @param height - The render buffer height.
	 * @param attachment - The attachment point of the frame buffer.
	 */
	public void attachRenderBuffer(int internalFormat, int width, int height, int attachment){
		bind();
		...
		GL30.glFramebufferRenderbuffer(GL30.GL_FRAMEBUFFER, attachment, GL30.GL_RENDERBUFFER, handle);
	...
	/**
	 * @return True if glCheckFramebufferStatus(GL_FRAMEBUFFER) = GL_FRAMEBUFFER_COMPLETE
	 */
	 ...
	 * @param a - Amount of alpha (0.0 - 1.0).
	 */
	 ...
	 * @param zNear - Map near plane to in window coordinates. (0.0 - 1.0 utilizes full depth buffer range).
	 * @param zFar - Map far plane to in window coordinates. (0.0 - 1.0 utilizes full depth buffer range).
	 */
	 ...
	 * @param color - Set pixel color value to clear color.
	 * @param depth - Set pixel depth value to depth value.
	 * @param stencil - Set pixel stencil value to stencil value.
	 */
	public static void clear(boolean color, boolean depth, boolean stencil){
		GL11.glClear((color ? GL11.GL_COLOR_BUFFER_BIT : 0) | (depth ? GL11.GL_DEPTH_BUFFER_BIT : 0) | (stencil ? GL11.GL_STENCIL_BUFFER_BIT : 0));
	}
