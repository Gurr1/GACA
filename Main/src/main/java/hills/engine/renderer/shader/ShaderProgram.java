...
public enum ShaderProgram {
	...
	private void bindAttributes() {
		for (ShaderAttribute attrib : ShaderAttribute.values())
			GL20.glBindAttribLocation(handle, attrib.getLocation(),
					attrib.getName());
	}
	...
	private void mapUniformBlocks() {
		...
		int uniformBlocks = GL20.glGetProgrami(handle,
		...
		for (int blockIndex = 0; blockIndex < uniformBlocks; blockIndex++) {
			...
			if (ShaderProgram.UBOs.map.get(name) != null) {
				GL31.glUniformBlockBinding(handle, blockIndex,
						ShaderProgram.UBOs.map.get(name).getBindPoint());
				continue;
			}

			// Get size of uniform block
			int size = GL31.glGetActiveUniformBlocki(handle, blockIndex,
					GL31.GL_UNIFORM_BLOCK_DATA_SIZE);

			// Get number of variables in uniform block
			int variables = GL31.glGetActiveUniformBlocki(handle, blockIndex,
					GL31.GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS);

			// Get indices of variables in uniform block
			IntBuffer variableIndices = MemoryUtil.memAllocInt(variables);
			GL31.glGetActiveUniformBlockiv(handle, blockIndex,
					GL31.GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES,
					variableIndices);
			...
			for (int variable = 0; variable < variables; variable++) {
				variableNames[variable] = GL31.glGetActiveUniformName(handle,
						variableIndices.get(variable));
				variableOffsets[variable] = GL31.glGetActiveUniformsi(handle,
						variableIndices.get(variable), GL31.GL_UNIFORM_OFFSET);
			}
			...
			ShaderProgram.UBOs.map.put(name, new UniformBuffer(size,
					variableNames, variableOffsets));
	...
	private void bindTextureUniforms() {
		...
		for (SamplerUniform sampler : SamplerUniform.values())
			if ((location = GL20
					.glGetUniformLocation(handle, sampler.getName())) != -1)
	...
	/**
	 * Map uniforms in shader program not associated to a uniform buffer block
	 * or a sampler.
	 */
	...
	/**
	 * @param name
	 *            - Name of sampler to look for.
	 * @return True if sampler with specified name exists in this shader
	 *         program.
	 */
	...
	/**
	 * Disables this shader programs. (Sets active shader program to '0' AKA no
	...
	public void loadMatrix(int location, Mat4 matrix) {
		GL20.glUniformMatrix4fv(location, false,
				BufferUtil.storeDataInFloatBuffer(matrix.getValues()));
	}
	...
	/**
	 * Get uniform handle/location. Will throw IllegalArgumentException if the
	...
	/**
	 * Deletes this shader program. Should be called before the application
	 * terminates.
	 */
	private void delete() {
		disable();
