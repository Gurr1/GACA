...
public final class TerrainLoader {
	...
	public static GridMeshData loadGridMesh(int width, int depth,
			int terrainWidth, int terrainDepth) {
		...
		for (int tileZ = 0; tileZ < depth; tileZ++)
			for (int tileX = 0; tileX < width; tileX++) {
				...
				/*
				 * // For each tile create 4 vertices ver[currentTileIndex * 4 +
				 * 0] = new Vertex(new Vec3(tileX, 0.0f, tileZ), new
				 * Vec2((float) tileX / width, ((float) tileZ / depth)), new
				 * Vec3(tileX, tileZ, currentTileIndex)); ver[currentTileIndex *
				 * 4 + 1] = new Vertex(new Vec3(tileX + 1, 0.0f, tileZ), new
				 * Vec2((float) (tileX + 1) / width, ((float) tileZ / depth)),
				 * new Vec3(tileX, tileZ, currentTileIndex));
				 * ver[currentTileIndex * 4 + 2] = new Vertex(new Vec3(tileX +
				 * 1, 0.0f, tileZ + 1), new Vec2((float) (tileX + 1) / width,
				 * ((float) (tileZ + 1) / depth)), new Vec3(tileX, tileZ,
				 * currentTileIndex)); ver[currentTileIndex * 4 + 3] = new
				 * Vertex(new Vec3(tileX, 0.0f, tileZ + 1), new Vec2((float)
				 * tileX / width, ((float) (tileZ + 1) / depth)), new
				 * Vec3(tileX, tileZ, currentTileIndex));
				 */

				// For each tile create 4 vertices
				ver[currentTileIndex * 4 + 0] = new Vertex(new Vec3(tileX,
						0.0f, tileZ), new Vec2(0, 1), new Vec3(tileX, tileZ,
						currentTileIndex));
				ver[currentTileIndex * 4 + 1] = new Vertex(new Vec3(tileX + 1,
						0.0f, tileZ), new Vec2(1, 1), new Vec3(tileX, tileZ,
						currentTileIndex));
				ver[currentTileIndex * 4 + 2] = new Vertex(new Vec3(tileX + 1,
						0.0f, tileZ + 1), new Vec2(1, 0), new Vec3(tileX,
						tileZ, currentTileIndex));
				ver[currentTileIndex * 4 + 3] = new Vertex(new Vec3(tileX,
						0.0f, tileZ + 1), new Vec2(0, 0), new Vec3(tileX,
						tileZ, currentTileIndex));
