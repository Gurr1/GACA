...
public class Terrain {
	...
	public int[][] createHeightMap() {
		...
		BufferedImage image = new BufferedImage(WIDTH + 1, HEIGHT + 1,
		...
		try {
			details = ImageIO.read(new File(
					"src/main/resources/detailsNoise.png"));
			background = ImageIO.read(new File(
					"src/main/resources/backgroundNoise.png"));
			smallestNoise = ImageIO.read(new File(
					"src/main/resources/smallestNoise.png"));
			noise4 = ImageIO.read(new File("src/main/resources/Noise4.png"));
			details2 = ImageIO.read(new File(
					"src/main/resources/detailsNoise2.png"));
			background2 = ImageIO.read(new File(
					"src/main/resources/backgroundNoise2.png"));
			smallestNoise2 = ImageIO.read(new File(
					"src/main/resources/smallestNoise2.png"));
		...
		for (int x = 0; x <= WIDTH; x++) {
			for (int y = 0; y <= HEIGHT; y++) {
				...
				green = Math.pow(3 * green, 1 / ELEVATION_MODIFIER)
						- Math.pow(2 * green, 1 / ELEVATION_MODIFIER);
				...
				double multiplier = (WIDTH * HEIGHT / 4)
						- ((distanceToCenterX * distanceToCenterY
								+ Math.pow(distanceToCenterX, exp) + Math.pow(
								distanceToCenterY, exp)) / 4);
		...
		try {
			ImageIO.write(image, "png", new File(
					"src/main/resources/textures/testnoise.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		return matrix;
	...
	private double exponentCalc() {
		...
		exp = exp
				- (Math.pow(
						Math.sqrt(HEIGHT * WIDTH) * Math.sqrt(2056.0 / WIDTH),
						0.0448646631));
		return exp;
	}
}
